...
class Setting extends Feature {
	...
	//무료지만 하루에 10000글자 제한
	private $naver_api_url = "https://openapi.naver.com/v1/language/translate";
	...
	/**
	 * 워드프레스의 미디어 라이브러리에는 파일 목록을 확장자 별로 볼 수 있는 필터가 있는데, 여기에 플러그인에서 추가한 확장자를 추가하는 펑션
	 *
	 * @param array $post_mime_types 미디어 라이브러리의 필터 파일 확장자 목록
	 * @return array 미디어 라이브러리의 필터 파일 확장자 목록  + 플러그인에서 추가한 확장자 명
	 */
	function media_filter_allow_mime_types( $post_mime_types ) {
		...
		foreach($plugin_allow_mime_types as $extension => $mime_type):
			...
			$post_mime_types[$mime_type] = array( __( $extension_name, 'RocketMLMT' ), __( 'Manage '.$extension_name, 'RocketMLMT' ), _n_noop( $extension_name . ' <span class="count">(%s)</span>', $extension_name .' <span class="count">(%s)</span>', 'RocketMLMT' ) );
	...
	/**
	 * 파일 업로드시 실행되는  워드프레스 필터중 하나
	 * 파일명이 한글인지 확인 후 한글이면 영문 혹은 영문으로 번역된 파일명을 반환
	 *
	...
	function rocket_sanitize_chars($filename) {
		...
		if($options['change_filename_kr_to_en']=="on"):
			...
			*/
			$check_file_name = sanitize_title_with_dashes($filename);
